 ...
 *
 * 또는 프로그램적:
 *   const at = WageGuard.start({endpoint: '/api/m6/probe'});
 *   const result = await at.probe();
...
(function (global) {
    ...
    async function _canvasHash() {
        try {
            ...
            ctx.font = '14px Arial';
            ctx.fillStyle = '#069';
            ctx.fillText('WageGuard probe fingerprint \u{1F50C}', 2, 2);
    ...
    function attachFormGuard(selector) {
        ...
        f.addEventListener('submit', async e => {
            if (f.dataset.WageGuardPassed === '1') return;
            e.preventDefault();
            const r = await probe();
            window.WageGuard._lastResult = r;
            const ev = new CustomEvent('WageGuard:result', { detail: r });
            document.dispatchEvent(ev);
            if (r.score >= config.threshold) {
                if (typeof window.WageGuardOnSuspicious === 'function') {
                    window.WageGuardOnSuspicious(r);
                } else {
                    alert(`⚠ 추가 본인확인이 필요합니다 (위험점수 ${r.score})\n사유: ${(r.factors || []).slice(0,3).map(x => x.label).join(' / ')}`);
                }
                return;
            }
            f.dataset.WageGuardPassed = '1';
            f.submit();
        });
    }
    ...
    global.WageGuard = WageGuard;
    global.PayRadar = WageGuard; // 하위 호환

    if (config.autoStart) {
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => WageGuard.start());
        } else {
            WageGuard.start();
        }
    }
